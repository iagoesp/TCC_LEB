...
float color[3] = { 1, 1, 1 };
float color_backGround[3] = { 1, 1, 1 };
...
void ConfigureTerrainProgram(GLuint glp, GLuint offset)
{
    ...
    glProgramUniform3f(glp, g_gl.uniforms[UNIFORM_TERRAIN_HEIGHT0 + offset], color_backGround[0],color_backGround[1],color_backGround[2]);
    //glProgramUniform3fv(glp, g_gl.uniforms[UNIFORM_TERRAIN_HEIGHT0 + offset], 1, color_backGround);
    glProgramUniform1f(glp,
...
float obterValorRuido(glm::vec3 pos){
    ...
    if(noise == RIDGED || noise == RIDGEDFBM){
        noiseRMF = Simplex::ridgedMF(wavelengthRMF*pos, ridgeOffset, octavesRMF, lacunarityRMF, gainRMF)/(float)wavelengthRMF;
            //LOG("noiseFBM %f\n", noiseFBM);
        if(noiseRMF<0 && floorf(getValleyRMF)){
            noiseRMF*=-1;
            noiseRMF = pow((float)noiseRMF, (float)getValleyRMF);
            //noiseFBM*=-1;
        }
        else
            noiseRMF = pow((float)noiseRMF, (float)getValleyRMF);
        ...
        if(noise == RIDGED)
            return (noiseRMF)/(1.0/(float)wavelengthRMF);
    }

    noiseFBM = Simplex::fBm(wavelengthFBM*pos, octavesFBM, lacunarityFBM, gainFBM)/(float)wavelengthFBM;
            //LOG("noiseFBM %f\n", noiseFBM);
    if(noiseFBM<0 && floorf(getValleyFBM)){
        noiseFBM*=-1;
    ...
    noiseRMF = (noiseRMF)/(1.0/(float)wavelengthRMF);
    if(noise == FBM)
        return noiseFBM;
    
    return glm::max(noiseFBM, noiseFBM + noiseRMF);
}
...
/*     int w = g_terrain.dmap.width/10;
    ...
    std::thread th8(gerarAltura, sz7, w, w, tamAmostra, zf_arr);
    ...
    if (g_app.viewer.hud) {
        ...
        ImGui::Begin("Ridged Setting");
        {
            if (ImGui::SliderFloat("Ridge size", &ridgeOffset, 0.f, 2.8f, "%0.01f"))
                LOG("Ridge Size = %f\n", ridgeOffset);
            if (ImGui::SliderFloat("Valley", &getValleyRMF, 0.0f, 8.0f, "%0.01f"))
                LOG("getValleyFBM = %i\n", getValleyRMF);  
        ...
        ImGui::SetNextWindowPos(ImVec2(g_framebuffer.w - 210, 420), ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(200, 200), ImGuiCond_FirstUseEver);
        ImGui::Begin("Color Picker");
        ImGui::ColorPicker3("ColorPicker3", (float*)&color);
        {
            if (ImGui::Button("Pick")){
                color_backGround[0] = color[0];
                color_backGround[1] = color[1];
                color_backGround[2] = color[2];
                LOG("color0 %f\n", color_backGround[0]);
                LOG("color1 %f\n", color_backGround[1]);
                LOG("color2 %f\n", color_backGround[2]);
